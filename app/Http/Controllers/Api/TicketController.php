...
class TicketController extends Controller
{
    ...
    public function invoice(int $id)
    {
        ...
        return response()->json([
            'data' => $this->buildTicketInvoice($registration),
        ]);
    }
    ...
    private function buildTicketInvoice(EventAttendee $registration): array
    {
        $event = $registration->event;
        $ticket = $registration->ticket;
        $contactInfo = is_array($event?->contact_info) ? $event->contact_info : [];
        $metadata = is_array($registration->attendee_metadata) ? $registration->attendee_metadata : [];
        $lineItem = is_array(data_get($metadata, 'line_item_fee_breakdown')) ? data_get($metadata, 'line_item_fee_breakdown') : [];
        $feeBreakdown = is_array(data_get($metadata, 'fee_breakdown')) ? data_get($metadata, 'fee_breakdown') : [];
        $currency = (string) ($event?->currency ?? data_get($feeBreakdown, 'currency') ?? 'UGX');
        $baseAmount = (float) ($registration->amount_paid ?? $registration->price_paid_ugx ?? data_get($lineItem, 'unit_price_ugx') ?? 0);
        $serviceFee = (float) data_get($lineItem, 'platform_commission_amount', 0) + (float) data_get($lineItem, 'processing_fee_amount', 0);
        $totalAmount = (float) (data_get($lineItem, 'total_amount') ?? ($baseAmount + $serviceFee));
        $taxRatePercent = max(0, (float) data_get($contactInfo, 'tax_rate_percent', 0));
        $taxIsInclusive = (bool) data_get($contactInfo, 'tax_is_inclusive', false);
        $taxAmount = 0.0;

        if ($taxIsInclusive && $taxRatePercent > 0 && $totalAmount > 0) {
            $taxAmount = round($totalAmount - ($totalAmount / (1 + ($taxRatePercent / 100))), 2);
        }

        return [
            'invoice_number' => 'INV-'.Str::upper($registration->confirmation_code),
            'issued_at' => $registration->created_at?->toIso8601String(),
            'currency' => $currency,
            ...
            'ticket' => [
                ...
                'order_id' => data_get($metadata, 'order_id'),
            ],
            'issuer' => [
                'name' => data_get($contactInfo, 'invoice_issuer_name')
                    ?: data_get($event?->resolveOrganizerUser(), 'name')
                    ?: $event?->title,
                'support_email' => data_get($contactInfo, 'invoice_support_email')
                    ?: data_get($contactInfo, 'support_email')
                    ?: data_get($event?->resolveOrganizerUser(), 'email'),
                'support_phone' => data_get($contactInfo, 'support_phone'),
                'tax_registration_number' => data_get($contactInfo, 'tax_registration_number'),
            ],
            'buyer' => [
                'name' => $registration->attendee_name,
                'email' => $registration->attendee_email,
            ...
            'line_items' => [[
                'label' => trim(implode(' - ', array_filter([$event?->title, $ticket?->name]))),
                'quantity' => 1,
                'unit_price' => round((float) (data_get($lineItem, 'unit_price_ugx') ?? $baseAmount), 2),
            ...
            'tax' => [
                ...
                'notes' => data_get($contactInfo, 'tax_vat_notes'),
            ],
            'totals' => [
