...
class EventResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'artists' => $this->resolveArtistsForResource(),
            ...
            'operations' => [
                ...
                'invoice_issuer_name' => data_get($this->contact_info, 'invoice_issuer_name'),
                'invoice_support_email' => data_get($this->contact_info, 'invoice_support_email'),
                'tax_registration_number' => data_get($this->contact_info, 'tax_registration_number'),
                'tax_rate_percent' => data_get($this->contact_info, 'tax_rate_percent'),
                'tax_is_inclusive' => data_get($this->contact_info, 'tax_is_inclusive'),
    ...
    private function resolveArtistsForResource(): array
    {
        $artists = collect();

        if ($this->relationLoaded('artists')) {
            $artists = $artists->merge($this->artists);
        }
        ...
        return $artists
            ...
            ->values()
            ->map(function ($artist) {
                $artistUser = $artist->relationLoaded('user') ? $artist->user : $artist->user()->first();

                return [
                    'id' => (string) $artist->id,
                    'name' => $artist->stage_name ?: $artist->name,
                    'slug' => $artist->slug,
                    'avatar_url' => StorageHelper::avatarUrl($artist->avatar, $artist->stage_name ?: $artist->name),
