...
class UpdateLoyaltyCardRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'tiers' => ['sometimes', 'array', 'min:1', 'max:4'],
            'tiers.*.name' => ['required_with:tiers', 'string', 'max:100'],
            'tiers.*.price_monthly' => ['required_with:tiers', 'numeric', 'min:0'],
            'tiers.*.price_yearly' => ['required_with:tiers', 'numeric', 'min:0'],
            'tiers.*.benefits' => ['required_with:tiers', 'array'],
